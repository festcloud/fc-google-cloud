...
def checkoutAndStash() {
    withCredentials([string(credentialsId: 'github-token', variable: 'GIT_TOKEN')]) {
        checkout scm: [
            ...
            userRemoteConfigs: [[url: "https://$GIT_TOKEN@github.com/festcloud/fc-google-cloud.git"]]
        ]
...
def deployFolder(String fusionUrl) {
    ...
    ansiColor('xterm') {
        withCredentials([file(credentialsId: 'sa-key', variable: 'SA_KEY')]) {
            ...
            jq '.properties' target/${ARTIFACT}-\$(cat version.txt).json > target/properties.json
            ...
            --data-binary @target/${ARTIFACT}-\$(cat version.txt).jar \\
            ...
            curl -X PUT \\
            -H "Authorization: Bearer \$(gcloud auth print-access-token)" \\
            --data-binary @target/properties.json \\
